int log2_ceil(int n) {//returns x s.t -> 1ll << x >= n
	int x = 0;
	while ((1ll << x) < (int)(n)) {
		x++;
	}
...
template<class Monoid, Monoid (*f)(Monoid, Monoid), Monoid (*e)()> struct SegmentTree {
	SegmentTree(int n) {
		_n = n;
